...
/* .footer {
    ...
    text-align: center;
    padding: 20px;
    clear: both; /* Ensure proper flow by clearing both sides */
} */
...
<body>
    ...
    <div class="navbar">
        ...
        if (isset($_SESSION["emp_id"]) || !isset($_SESSION["emp_id"])) {
            // Fetch and calculate average scores for gender = female
            $femaleData = fetchAndCalculateAverage($conn, 'female');
            $maleData = fetchAndCalculateAverage($conn, 'male');
           ...
           $age1930Data = fetchAndCalculateAverageByAgeRange($conn, 19, 30);
           $age3140Data = fetchAndCalculateAverageByAgeRange($conn, 31, 40);
           $age4150Data = fetchAndCalculateAverageByAgeRange($conn, 41, 50);
           $age5160Data = fetchAndCalculateAverageByAgeRange($conn, 51, 60);
           $age60plusData = fetchAndCalculateAverageByAgeRange($conn, 61, PHP_INT_MAX);
            echo "<script>let averageScores = " . json_encode($averageScores) . ";</script>";
            // Pass the combined data to JavaScript
            echo "<script>let femaleData = " . json_encode($femaleData) . ";</script>";
            echo "<script>let maleData = " . json_encode($maleData) . ";</script>";
    ...
    echo "<script>let age60plusData = " . json_encode($age60plusData) . ";</script>";
        ...
        function fetchAndCalculateAverageByAgeRange($conn, $minAge, $maxAge) {
            $sql = "SELECT u.*, s.*
                    FROM users u
            ...
            return fetchAndCalculateAverage1($conn, $sql, $minAge, $maxAge, $minAge, $maxAge);
        }
        ...
        function fetchAndCalculateAverage($conn, $gender) {
            $sql = "SELECT u.*, s.*
                FROM users u
                LEFT JOIN score s ON u.emp_id = s.emp_id
                WHERE u.gender = ?
            ...
            $prepareStmt = mysqli_stmt_prepare($stmt, $sql);
            if ($prepareStmt) {
                mysqli_stmt_bind_param($stmt, "ss", $gender, $gender);
                mysqli_stmt_execute($stmt);
                $result = mysqli_stmt_get_result($stmt);
    ...
    ?>
     <?php echo "<h1 style='color: white; text-align: center; font-size: 1.5em;'>Analysis of University</h1>"?><br><br>
...
<div class="chart-container2">
       <?php echo "<h2 style='color: white; text-align: center; font-size: 1.5em;'>Pie Chart - Health Scores Overall</h2> <h2 style='color: white; text-align: center; white-space: nowrap; font-size: 1.5em;'>Grouped Bar Chart - Health Scores by Gender</h2>"; ?>
        ...
        <br><br>
        <?php echo "<h2 style='color: white; text-align: center; font-size: 1.5em;'>Grouped Bar Chart - Health Scores by University Role</h2>"?>
...
<br><br>
        ...
        <script src="drawoverall.js"></script>
    </body>
    </html>
